...
/* History: each finished evaluation is appended (never overwritten) so the leader can audit past results */
async function getHistory(name){return (await sGet('hist:'+name))||[];}
async function appendHistory(name,record){
  const hist=await getHistory(name);
  hist.push(record);
  await sSet('hist:'+name,hist);
  return hist;
}

/* ============ STATE ============ */
let S={screen:'home',currentUser:null,conceptIndex:0,order:[],answers:[],selectedOption:null,revealed:false,pinError:'',detailUser:null,detailResult:null,dashboardResults:{},historyUser:null,historyList:[],historyIndex:null,historyRecord:null};
...
/* Pick a "nice" step (1,2,5,10,20,...) so the Y axis shows ~5-6 readable labels regardless of max */
function niceStep(max,targetTicks=6){
  const raw=max/targetTicks;
  const mag=Math.pow(10,Math.floor(Math.log10(raw)));
  const norm=raw/mag;
  let n;
  if(norm<1.5)n=1; else if(norm<3)n=2; else if(norm<7)n=5; else n=10;
  return n*mag;
}
...
function drawBarChart(canvas, labels, values, max, barColors){
  ...
  // grid lines + Y labels (spaced with a "nice" step to avoid overlap)
  const step=niceStep(max);
  for(let v=0;v<=max+0.0001;v+=step){
    const vr=Math.round(v*100)/100;
    const y=PAD.top+cH-(vr/max)*cH;
    ctx.strokeStyle= vr===0?'#2C4368':'#1E3456';
    ctx.lineWidth=vr===0?1.5:1;
    ctx.beginPath();ctx.moveTo(PAD.left,y);ctx.lineTo(PAD.left+cW,y);ctx.stroke();
    ctx.fillStyle='#8FA3C4';ctx.font=`11px 'JetBrains Mono', monospace`;
    ctx.textAlign='right';ctx.fillText(vr,PAD.left-6,y+4);
  }
...
function render(){
  ...
  if(S.screen==='historyDetail')return renderHistoryDetail();
}
...
window.nextStation=async function(){
  ...
  if(idx===CONCEPTS.length-1){
    ...
    await sSet('res:'+S.currentUser,res);
    await appendHistory(S.currentUser,res);
    S.screen='results';
...
window.resetUser=async function(n){if(!confirm(`¿Seguro que querés reiniciar la autoevaluación de ${n}? El historial de evaluaciones anteriores se conserva.`))return;await sDel('res:'+n);S.detailResult=null;render();};
...
function renderDetail(){
  ...
  app.innerHTML=`${topBar()}<button class="back-link" onclick="goDashboard()">← Volver al equipo</button>
    ...
    <div class="footer-actions">
      ...
      <button class="btn btn-ghost" onclick="goHistory('${sn}')">📜 Ver historial</button>
...
/* HISTORY LIST (manager only — reached only from the PIN-protected detail screen) */
window.goHistory=async function(n){
  S.historyUser=n;
  S.historyList=await getHistory(n);
  S.screen='history';
  render();
};
function renderHistory(){
  const n=S.historyUser,hist=S.historyList||[];
  const sn=n.replace(/'/g,"\\'");
  let cards='';
  if(!hist.length){
    cards=`<p style="color:var(--text-dim);margin-top:8px">Todavía no hay autoevaluaciones registradas para ${n}.</p>`;
  }else{
    hist.slice().reverse().forEach((r,ri)=>{
      const i=hist.length-1-ri; // original index, most recent first in display
      cards+=`<div class="team-card" onclick="viewHistoryRecord(${i})"><span class="num">#${i+1} de ${hist.length}</span><div class="name">${r.date}</div><span class="score-pill">${r.total} / ${MAX} pts &nbsp;·&nbsp; ${pct(r.total,MAX)}</span></div>`;
    });
  }
  app.innerHTML=`${topBar()}<button class="back-link" onclick="goDetail('${sn}')">← Volver al detalle</button>
    <div class="hero" style="padding-top:6px"><span class="eyebrow">${n}</span><h1>Historial de autoevaluaciones</h1><p>Se guarda un nuevo registro cada vez que ${n} finaliza una autoevaluación. Total: ${hist.length} registro${hist.length===1?'':'s'}.</p></div>
    <div class="team-grid">${cards}</div>`;
}

window.viewHistoryRecord=function(index){
  S.historyIndex=index;
  S.historyRecord=S.historyList[index];
  S.screen='historyDetail';
  render();
};
window.backToHistory=function(){S.screen='history';render();};
window.exportHistoryExcel=function(){exportIndividualExcel(S.historyUser,S.historyRecord);};
window.exportHistoryDashboard=function(){exportIndividualDashboard(S.historyUser,S.historyRecord);};

function renderHistoryDetail(){
  const n=S.historyUser,r=S.historyRecord,idx=S.historyIndex,total_=S.historyList.length;
  let bars='';
  r.answers.forEach(a=>{const p=Math.round((a.score/5)*100);bars+=`<div class="bar-row"><div class="bar-label">${a.title}</div><div class="bar-track"><div class="bar-fill ${scc(a.score)}" style="width:${p}%"></div></div><div class="bar-val">${a.score}</div></div>`;});
  const topS=Math.max(...r.answers.map(a=>a.score));
  app.innerHTML=`${topBar()}<button class="back-link" onclick="backToHistory()">← Volver al historial</button>
    <div class="results-head"><span class="eyebrow">${n} · Registro #${idx+1} de ${total_}</span><h1>Autoevaluación del ${r.date}</h1></div>
    <div class="kpi-strip">
      <div class="kpi"><div class="kv">${r.total}</div><div class="kl">Puntaje total<br>de ${MAX} posibles</div></div>
      <div class="kpi"><div class="kv">${pct(r.total,MAX)}</div><div class="kl">Del puntaje<br>máximo</div></div>
      <div class="kpi"><div class="kv" style="font-size:18px">${lvl(topS)}</div><div class="kl">Nivel<br>predominante</div></div>
    </div>
    ${bars}
    <div id="chart-historydetail"></div>
    <div class="footer-actions">
      <button class="btn btn-ghost" onclick="backToHistory()">Volver al historial</button>
      <button class="btn btn-ghost" onclick="exportHistoryExcel()">⬇ Datos en Excel</button>
      <button class="btn btn-primary" onclick="exportHistoryDashboard()">⬇ Dashboard con Gráficos</button>
    </div>`;
  mountChart(document.getElementById('chart-historydetail'),'Puntaje por competencia',r.answers.map(a=>a.title.split(' ').slice(0,2).join(' ')),r.answers.map(a=>a.score),5,r.answers.map(a=>scoreColor(a.score)));
}
